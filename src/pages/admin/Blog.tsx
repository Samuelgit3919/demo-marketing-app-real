...
const AdminBlog = () => {
  ...
  if (checkingAuth || !session) {
    return <><AdminTopBar /><div className="min-h-screen bg-brand-cream flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-brand-copper" /></div></>;
  }
  if (!isAdmin) {
    return <><AdminTopBar /><div className="min-h-screen bg-brand-cream flex items-center justify-center px-6"><Card className="p-8 text-center border-brand-border"><h2 className="text-2xl font-semibold text-brand-espresso mb-2">Access Denied</h2><p className="text-brand-muted">You don't have admin privileges.</p></Card></div></>;
  }
