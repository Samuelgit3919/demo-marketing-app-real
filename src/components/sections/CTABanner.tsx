...
import { SITE_KEYS, DEFAULT_CTA } from "@/lib/siteContent";

export default function CTABanner() {
  const { content: c } = useSiteContent(SITE_KEYS.cta, DEFAULT_CTA);
  return (
    <section className="relative overflow-hidden h-[500px] flex items-center">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-10 text-center">
        <span className="text-brand-copper-light text-xs tracking-[0.3em] uppercase block mb-6">{c.eyebrow}</span>
        <h2
          className="text-white font-light mb-8"
          style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(2.2rem, 5vw, 4rem)" }}
        >
          {c.headingLine1}<br />
          <em className="text-brand-copper-light not-italic">{c.headingEmphasis}</em>
        </h2>
